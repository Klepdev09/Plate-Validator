...
/** Canonical US state list keyed by FIPS (matches us-atlas geography ids). */
...
export function getStateByFips(fips: string | null): USState | undefined {
  if (!fips) return undefined
  return US_STATES.find((state) => state.fips === fips)
}

export function normalizeFips(id: string | number | undefined): string {
